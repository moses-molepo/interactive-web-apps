...
for (let i = 0; i < extracted.length; i++) {
  ...
  sneakPeak.innerHTML = `
    <div>
      <img class='preview__image' src="${books[i].image}" alt="No picture available" />
    </div>
...
if (result.length >= 1) {
  for (let i = 0; i < result.length; i++) {
    ...
    sneakPeak.dataset.subtitle = `${authors[result[i].author]} (${new Date(result[i].published).getFullYear()})`;
    sneakPeak.dataset.description = result[i].description;
    sneakPeak.dataset.genre = result[i].genres;

    sneakPeak.innerHTML = `
      <div>
        <img class='preview__image' src="${result[i].image}" alt="No picture available" />
      </div>
      <div class='preview__info'>
